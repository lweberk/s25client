...
struct RoadsBuilding
{
    ...
    std::vector<uint8_t> route;  ///< Richtungen der gebauten Straße
};
    ...
    public LobbyInterface
{
    ...
    public:
        ...
        uint32_t TestBuiltRoad(const MapPoint pt);
        ...
        void ShowRoadWindow(int32_t mouse_x, int32_t mouse_y);
        /// Zeigt das Actionwindow an, bei Flaggen werden z.B. noch berücksichtigt, obs ne besondere Flagge ist usw
        void ShowActionWindow(const iwAction::Tabs& action_tabs, MapPoint cSel, int32_t mouse_x, int32_t mouse_y, const bool enable_military_buildings);
    ...
    private:
        ...
        void DemolishRoad(const uint32_t start_id);

        /// Updatet das Post-Icon mit der Nachrichtenanzahl und der Taube
        void UpdatePostIcon(const uint32_t postmessages_count, bool showPigeon);
        ...
        void CI_NewPostMessage(const uint32_t postmessages_count);
        void CI_PostMessageDeleted(const uint32_t postmessages_count);
        ...
        void GI_PlayerDefeated(const uint32_t player_id);
        ...
        void GI_Winner(const uint32_t player_id);
        void GI_TeamWinner(const uint32_t player_id);
};
