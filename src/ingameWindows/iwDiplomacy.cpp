...
iwDiplomacy::iwDiplomacy()
    : IngameWindow(CGI_DIPLOMACY, (uint16_t) - 1, (uint16_t) - 1, 500, FIRST_LINE_Y + GAMECLIENT.GetPlayerCount() * (CELL_HEIGHT + SPACE_HEIGHT) + 20, _("Diplomacy"),
...
void iwDiplomacy::Msg_PaintAfter()
{
    ...
    for(uint32_t i = 0; i < GAMECLIENT.GetPlayerCount(); ++i)
    {
        ...
        if(GetCtrl<ctrlText>(500 + i))
        {
            ...
            {
                uint32_t duration = GAMECLIENT.GetLocalPlayer()->GetRemainingPactTime(PactType(z), i);
...
void iwDiplomacy::Msg_ButtonClick(const uint32_t ctrl_id)
{
    ...
    {
        uint8_t player_id = static_cast<uint8_t>(ctrl_id - 300);
    ...
    {
        uint8_t player_id = static_cast<uint8_t>(ctrl_id - 400);
        // Noch kein Bündnis abgeschlossen?
...
const uint32_t DURATION_COUNT = 3;

/// Längen für die Dauer des Vertrages (kurz-, mittel- und langfristig)
const uint32_t DURATIONS[DURATION_COUNT] =
{
    5000,
    30000,
    100000
...
iwSuggestPact::iwSuggestPact(const PactType pt, const uint8_t player) : IngameWindow(CGI_SUGGESTPACT, (uint16_t) - 1,
            (uint16_t) - 1, 320, 215, _(PACT_TITLES[pt]), LOADER.GetImageN("resource", 41)), pt(pt), player(player)
{
...
void iwSuggestPact::Msg_ButtonClick(const uint32_t ctrl_id)
{
    /// Dauer auswählen (wenn id == DURATION_COUNT, dann "für alle Ewigkeit" ausgewählt)
    uint32_t selected_id = GetCtrl<ctrlComboBox>(6)->GetSelection();
    uint32_t duration = (selected_id == DURATION_COUNT) ? 0xFFFFFFFF : DURATIONS[selected_id];
