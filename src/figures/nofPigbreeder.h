...
class nofPigbreeder : public nofWorkman
{
        ...
        uint16_t GetCarryID() const { return 73; }
        /// Der Arbeiter erzeugt eine Ware
        GoodType ProduceWare();

    public:

        nofPigbreeder(const MapPoint pt, const uint8_t player, nobUsual* workplace);
        nofPigbreeder(SerializedGameData* sgd, const uint32_t obj_id);
