...
class nofFarmer : public nofFarmhand
{
    ...
    private:
        ...
        bool checkSurrounding(const MapPoint pt, int32_t type);

    public:

        nofFarmer(const MapPoint pt, const uint8_t player, nobUsual* workplace);
        nofFarmer(SerializedGameData* sgd, const uint32_t obj_id);
